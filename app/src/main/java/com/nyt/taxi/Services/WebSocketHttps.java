...
//                h[8] = (msg_size >>  0) & 0xff;
    ...
    private void sendCoordinateToServer() {
        Point p = new Point(UPref.getFloat("last_lat"), UPref.getFloat("last_lon"));
        if (p.getLatitude() > 1 && p.getLongitude() > 1 && !UPref.getBoolean("paused")) {
                ...
                case DriverState.OnWay:
                    ...
                    webintent.putExtra("msg",
                            String.format("{\"data\":{\"lat\": %s, \"lut\": %s, \"speed\":%s, \"azimuth\":%s},\"event\": \"client-broadcast-api/update-coordinate\",\"channel\": \"%s\"}",
                                    UText.valueOf(p.getLatitude()),
                                    UText.valueOf(p.getLongitude()),
        ...
        intent.putExtra("longtitude", p.getLongitude());
